...
Requires Node.js 22+ and Foundry (or a verified contract address for [ABI mode](#use-without-foundry-abi-mode)). Install the [npm preview](https://www.npmjs.com/package/clear-signing-helper/v/0.5.0-preview.1):
...
Pin `clear-signing-helper@0.5.0-preview.1` for reproducible installs. This is an experimental developer preview; specify the preview tag or exact version explicitly.

Expected version: `0.5.0-preview.1`. Installation is independent of your contract repository; it does not need a `package.json` or Node dependencies. Have `forge` available on your PATH.
...
```sh
git clone --depth 1 --branch v0.5.0-preview.1 \
...
- **ERC-20 and WETH conventions.** A contract with the full ERC-20 surface gets the registry's usual `approve`, `transfer` and `transferFrom` formats denominated in the contract itself; a WETH-shaped contract also gets `Wrap` and `Unwrap`. These are conventions, marked as such, and editable like any draft. The `interpolatedIntent` uses the registry's phrasing when it fits 30 characters with the contract's parameter names, a shorter one when it does not (`Let {spender} spend {value}`), and none when nothing fits; provenance says which.
...
`--address` is the one network call `init` makes, and only when asked. It fetches the verified source match from [Sourcify](https://sourcify.dev), falling back to Etherscan V2 when `ETHERSCAN_API_KEY` is set. A verified proxy is resolved to its implementation: the descriptor describes the implementation's functions and binds the proxy address users actually call. NatSpec from the verified source feeds intents and labels the same way Foundry artifacts do. Address immutables with a public getter, such as a staking contract's `stakingToken`, become `metadata.constants` (tagged `verified source` in provenance), so an amount can be denominated as `$.metadata.constants.stakingToken`. Sourcify's record carries each immutable's value in the verified bytecode but no AST to name it. The tool names it from the verified source map: every place the bytecode reads the immutable maps to its declaration or its identifier. An immutable whose references disagree, whose value is not an address, or that has no getter is left out rather than guessed. Nothing extra is fetched: the same Sourcify request carries the bytecode and sources. If that record is too large, the ABI is imported without constants. The verified source files from the same record are written to `clear-signing/source/<Name>/` for reading while you decide intents and denominations; nothing reads them and they are not fingerprinted. `--registry <registry-clone>` makes `init` compare selectors against that clone instead of the bundled snapshot, so a descriptor merged after this release is found too. An unverified address is refused with the reason; verify it on Sourcify or pass the ABI you trust with `--abi`.
...
The template lists every function and every argument leaf with its current value and read-only hints: the NatSpec text, the registry's formats for the same selector, candidate denominations for amounts (`@.to`, `$.metadata.constants.*`, address arguments, a literal token), and the formats valid for the type. At the top, `owner`, `contractName`, `url` and `enums` (enum name to `{"value": "Label"}`, referenced from a field as `{"$ref": "$.metadata.enums.<name>"}`) go to `metadata`. In a descriptor without groups, fields are written in the order of their keys, so moving a key moves the field. `receive()` has no entry (it needs none); `fallback()` has an exclude-only one. `apply` validates the result before writing anything, then writes the descriptor, `exclusions` and `hidden` in `clear-signing.toml` (a hidden argument stays in the descriptor as a `visible: "never"` field, as registry descriptors write it, so the registry linter counts it and reviewers see it; the reason stays in `clear-signing.toml`), and records each decision in `clear-signing/provenance.json` as `human` or `llm` according to the file's `author`. Hidden native value and unsupported formats are refused. Running `decisions` again reproduces the current state and keeps the file's `author`, so the file round-trips. A later decision replaces the earlier one for the same slot in provenance, and a removed interpolated intent is recorded as removed.
...
| Command | Behavior |
| --- | --- |
| `init` | Scaffold selected contracts; existing descriptors are preserved. `--abi <file>` or `--address <addr> --chain-id <id>` starts ABI mode; `--registry <clone>` matches against a registry clone |
...
| `fixture` | Encode a sample call using the compiled ABI, or copy a real transaction: `--broadcast-tx <hash>` from a broadcast record, `--tx <hash> --rpc-url <url>` from the chain; `--rpc-url` also fills missing token metadata |
...
`check` prints warnings grouped by descriptor and code, the ones a signer would notice first (undisplayed arguments, token mapping, type mismatches), then what registry lint reports (intent length), then advice (registry priors), with each code's remedy once. An argument a displayed field reads through `tokenPath`, `token`, `collectionPath` or `chainIdPath` counts as displayed, as it does in the registry linter.

`init` also checks whether the registry already describes the contract. It compares the contract's function selectors with every registry calldata descriptor in the same snapshot and lists up to three matches ("Already in the registry?", `registryMatches` in `--json`). A match needs at least two shared functions, and either every function the file describes exists in the contract, which is what `registry add-deployment`'s selector proof requires, or the file covers at least half of the contract's functions. Functions that three or more registry entities describe (ERC-20, ERC-4626, ERC-721, Ownable, `multicall`) name an interface, not a protocol, and do not count toward the overlap. A match is a suggestion. It cannot tell the same protocol on a new chain (use `add-deployment`) from a fork, whose file belongs to another owner, and nothing is bound or skipped on it. Measured at registry `8f56072` and again at `53d86dc` (2026-09-25), with the same results: a Uniswap V2 router finds `quickswap/calldata-QuickSwap.json`, SwapRouter02 finds `uniswap/calldata-UniswapV3Router02.json`, and none of the 64 puddleswap artifacts other than router interfaces match anything.

`check` also compares each format against what other registry descriptors do with the same function selector, using a pinned snapshot of the registry's display formats. When every prior, from at least two registry entities, types or hides an argument and the draft leaves it raw or shown, `check` warns (`CORPUS_DISAGREEMENT`); a single entity's choice, often for an unrelated function that shares the selector, stays a hint; `init` lists the priors it found under the draft's evidence. This is advisory: the registry's choices for one contract are a strong hint for another with the same selector, not proof. Refresh the snapshot with `npm run priors:snapshot -- <registry-clone>`.
...
Export then formats the descriptors with the registry's own `erc7730 format` (the registry's format bot would otherwise rewrite them) and runs its linter through `uvx`, the way the registry's CI does: the `erc7730` package from the registry's `.github/requirements.txt` (a Sourcify fork pinned by commit) with the flags its pull-request workflow passes. The built-in pin follows the registry's CI after [#3038](https://github.com/ethereum/clear-signing-erc7730-registry/pull/3038), so `--require-verified` is on and every deployment must be verified on Sourcify; `--registry <registry-clone>` reads the pin from a clone instead. A lint error removes the bundle and fails the export; warnings are recorded in `review/validation.json`. Without `uvx` the exact command is printed instead, and `--no-lint` skips it. `check` already warns locally when an intent exceeds the linter's 30-character limit.

Add `--registry-runners` to also run the two implementations the registry's CI tests every submission against: the Sourcify TypeScript runner and the Rust `cs-test` runner, at the revisions the registry pins. The first use clones and builds them under `~/.cache/clear-signing-helper/runners/`, or `$CLEAR_SIGNING_RUNNERS_DIR` when set (needs `git`, `npm` and `cargo`; `clear-signing registry setup-runners` does this ahead of time). Every case must pass in both, or the export fails and the bundle is removed. The two implementations sometimes disagree with each other (at the pinned revisions the Rust runner names Polygon's native currency MATIC and the Sourcify runner POL, registry [#3027](https://github.com/ethereum/clear-signing-erc7730-registry/issues/3027)); then `--accept-runner-failure <sourcify|rust>=<reason>` exports as long as the other runner passes, keeps the failing runner's output, and records the reason in `review/validation.json` and the bundle README. `registry add-deployment --runners` takes the same option. Results and logs land in `review/runners/`. `--registry <registry-clone>` reads the runner revisions (and the lint pin) from a clone's CI definition instead of the built-in pins. `registry add-deployment --runners` does the same for the test file it updates, and restores the clone if any case fails.

The runners are required, not optional, when a displayed field is a signed integer or a nested array. On those shapes the expected values come from this tool's renderer, which carries a signed-integer fix and flattens nested arrays before rendering, and the registry's runners can render them differently. `export` and `registry add-deployment` then stop with `REGISTRY_RUNNERS_REQUIRED` unless the runners run, or `--skip-registry-runners "<reason>"` records why they did not; the reason goes into `review/validation.json` and the bundle README. Hiding the field (with a reason) also clears it.

`--out` names a new directory inside the project; an absolute path inside it is accepted, anything outside is refused with that reason (copy the bundle afterwards). Export refuses existing directories, escaping paths, two selected contracts with the same name, and conflicting metadata across fixtures for the same descriptor. Copy `registry/<entity>/` into a registry clone and open the pull request from an account tied to the contract owner. Deployed-code verification, registry review, attestations, and wallet distribution remain separate steps. [Submission guide](https://clearsigning.org/build/)
...
- Local metadata only. Fixtures may carry `tokens`, `addressNames`, `ensNames`, `nftCollectionNames`, `blockTimestamps`, and a `chain` entry with the native currency. Well-known chains, including Monad, fall back to a built-in native-currency table; anything else needs `chain` in the fixture, or `--chain-name` and `--native-currency` when creating it. Rendering fetches nothing; `fixture --rpc-url` is the one step that reads token metadata from a chain, into the fixture.
