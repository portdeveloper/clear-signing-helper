...
The owner selected MIT; the license text and package metadata are included in the candidate. Still required: owner scope approval, remote platform CI, protocol maintainer/human review, physical-device and intended descriptor-trust-path validation, and publication authorization. See [the handoff](../docs/RELEASE-HANDOFF.md). The general CLI does not verify deployed code, discover proxies, sign transactions, attest or publish.
