...
export interface AbiSource {source: 'file' | 'sourcify' | 'etherscan'; importedAt: string; origin: string; chainId?: number; address?: string; match?: string; url?: string; proxy?: VerifiedContract['proxy']; userdoc?: any; devdoc?: any}
...
export function importVerified(root: string, v: VerifiedContract, name?: string) {
  ...
  if (!nameOk(contractName)) fail('INVALID_NAME', `Contract name "${contractName}" must be a Solidity identifier. Pass --name.`, 2);
  return storeAbi(root, contractName, v.abi, {source: v.source, importedAt: v.fetchedAt, origin: v.url, chainId: v.chainId, address: v.address, match: v.match, url: v.url, ...(v.proxy ? {proxy: v.proxy} : {}), ...(v.userdoc ? {userdoc: v.userdoc} : {}), ...(v.devdoc ? {devdoc: v.devdoc} : {})});
}
...
export function loadAbiProject(root: string): Project {
  ...
  for (const file of walk(dir, '.json')) {
    ...
    if (source) assertKeys(source, ['source', 'importedAt', 'origin', 'chainId', 'address', 'match', 'url', 'proxy', 'userdoc', 'devdoc'], `${path.relative(root, sidecarFile)}`);
    ...
    contracts.push({id: abiContractId(name), name, source: `${ABI_DIR}/${name}.json`, artifact: file, abi, functions: functions.sort((a, b) => a.format().localeCompare(b.format())), special: abi.filter((x: any) => ['fallback', 'receive'].includes(x.type)).map((x: any) => `${x.type}()`), metadata: {}, userdoc: source?.userdoc, devdoc: source?.devdoc});
