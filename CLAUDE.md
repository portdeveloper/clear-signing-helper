...
- `fixture` encodes a call or copies a real transaction, from a broadcast record (`--broadcast-tx`) or the chain (`--tx --rpc-url`), keeping its hash as the exported `txHash`; `preview` renders through the vendored Sourcify renderer; `test` snapshots; `review --accept` records a fingerprinted acknowledgement; `check` validates (schema, selectors, paths, renderer-supported formats, registry-prior disagreement, review freshness), all with `--contract`.
...
It never signs, sends, publishes, verifies bytecode beyond a Sourcify match, or opens PRs. Network access only at `init --address`, `registry add-deployment` (Sourcify, or read-only RPC for EIP-712), `fixture --tx` (read-only RPC), the upstream lint/format run, and the one-time runner build. The agent skill under `.claude/skills/clear-signing-helper/` drives these commands; authoring a new EIP-712 descriptor still falls back to the upstream Python tool.
...
- Done 2026-09-25: real transactions in any project, as `fixture --tx <hash> --rpc-url <url>` rather than the discovery first planned here. `eth_getLogs` finds events, not calls: a function that emits nothing is invisible, a call made through a router or multicall shows the outer target, and scanning log ranges on public endpoints is slow and rate-limited. The user or agent supplies hashes (an explorer lists them). `fetchTransaction` in `src/fetch.ts` checks the chain from `eth_chainId`, and refuses a transaction that is pending (`TX_PENDING`), reverted (`TX_REVERTED`), a contract creation, or signed for another chain. `createFixture` then requires the target to be a bound deployment (`TX_TARGET_MISMATCH`, which names routers and multicalls), a write function (`TX_SELECTOR_MISMATCH`), and canonical calldata (`decodeCanonical`, now shared with the renderer), and writes nothing on refusal. Fixtures gained `txHash`, set by `--tx` and `--broadcast-tx` (a broadcast CALL is a real transaction), and exported as the registry test case's optional `txHash`. Live: the recorded `notifyRewardAmount` on Monad testnet (`0xe93c…96fc`, block 52286214) copied byte for byte into the address-mode StakingRewards project through `https://testnet-rpc.monad.xyz`; the WMON transfer from the same script was refused as `TX_TARGET_MISMATCH`. Automatic discovery stays possible later, if someone asks for it.
...
- Done 2026-09-25: `--registry <clone>` names the registry clone on `export`, `registry add-deployment` and `registry setup-runners`; `--ci-pins`, the former name on export, is a hidden alias on export and setup-runners, and the two together with different values are a `USAGE_ERROR`.
...
- Network access is limited to explicit, user-requested moments: `init --address` and `registry add-deployment` (Sourcify, then Etherscan with a key; for EIP-712 descriptors, read-only `eth_chainId`, `eth_getCode` and `eth_call` against the `--rpc-url` the user names, never a built-in endpoint), `fixture --tx` (read-only `eth_chainId`, `eth_getTransactionByHash` and `eth_getTransactionReceipt` against the `--rpc-url` the user names, added 2026-09-25), upstream `erc7730 lint` and `format` during export or add-deployment (via `uvx`, which installs the registry's pinned `erc7730` from GitHub on first use), and the one-time clone-and-build of the registry runners behind `--registry-runners` / `--runners` / `registry setup-runners`. Everything else stays offline. Record any new exception here.
